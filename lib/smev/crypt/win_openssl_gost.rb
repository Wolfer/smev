...
module Smev
  module Crypt
    module WinOpenSSL
      ...
      def digest text
        calculate_hash = Features::call_shell("#{cmd} dgst -engine gost -md_gost94 -binary", ( text.is_a?(String) ? text : text.canonicalize_excl ) )
      ...
      def verify_signature security
        ...
        public_key_file = Features::write_tmp Features::call_shell("#{cmd} x509 -engine gost -pubkey -noout", certificate)
        ...
        raise SignatureError.new("Wrong signature!") unless /OK/ =~ Features::call_shell( "#{cmd} dgst -engine gost -verify "+public_key_file.path+' -signature '+signature_file.path, sig_info )
      ensure
      ...
      def verify_file file
        /successful/ =~ Features::call_shell( "#{cmd} smime -verify -engine gost -noverify -inform DER -in #{file}.sig -content #{file}", '')
      end
